...
void CodeGenModule::AddGlobalSYCLIRAttributes(llvm::GlobalVariable *GV,
  ...
  const Optional<llvm::SmallSet<StringRef, 4>> AttrFilter =
      getAttributeFilters(A->args_begin(), A->args_size(), A);
  ...
  const auto NameValuePairs = getValidAttributeNameValuePairs(
...
CodeGenModule::getValidAttributeNameValuePairs(
    Expr **AttributeExprs, const size_t AttributeExprsSize,
  ...
  if ((AttributeExprsSize & 1) != 0)
    getDiags().Report(diag::err_sycl_add_ir_attribute_must_have_pairs)
        << Attribute;
...
CodeGenModule::getAttributeFilters(Expr **AttributeExprs,
